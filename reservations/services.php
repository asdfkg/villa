...
/* Added to get the Property Image on checkout page */
if(SITE_ID==1){
    
    $propertyGalleryArray = array();
    $propertyGallery = $propertyArray['property_name'].($propertyArray['property_gallery_show_all']?'':'/limited');
    ...
    {   
        $files = scandir ($path);
        $firstFile = $path . $files[2];
    }
    $propertyArray['property_checkout_img'] = str_replace('../','/',$firstFile);
}
// echo "<pre>"; print_R($propertyArray); die; 
/* End  */
?>
<!doctype html>
...
<!--        <div id="service-levels"></div>-->
    ...
    <script type="text/jsx">
        ...
        ReactDOM.render(
            <CheckoutStep2 property={propertyInfo} siteid={siteid} serviceInfo={services} moreServices={<?php echo json_encode($moreServices);?>} additionalServicesInfo={<?php echo json_encode($additionalServices);?>} selectedServiceLevel={selectedServiceLevel} totalNights="<?php echo $_SESSION['RESERVATION']->get('nightTotal');?>" />,
        ...
        ReactDOM.render(
            <ReservationServiceSection data={propertyInfo} siteid="<?php echo SITE_ID;?>" checkOut="<?php echo date('F jS, Y', strtotime($propertyArray['check_out_dt'])); ?>" 
	...
	function applyServices() {
		...
		$('.option input[type="number"]').each(function () {
			if ($(this).val()) {
                            if (Number($(this).val()) < Number($(this).data('min')) || Number($(this).val()) > Number($(this).attr('max'))) $(this).val('');
				...
				switch (data.result)
				{
					...
					case 1:
						...
						$('#checkoutBtn').removeAttr('disabled');
                                                $('#rateNight').val(rateNight);
                                                var additionalServiceCost = rateNight - rateBaseNight;
                                                $('#additional-service-cost').html("$"+additionalServiceCost+" / NIGHT");
					break;
				}
			},
			error: function()
